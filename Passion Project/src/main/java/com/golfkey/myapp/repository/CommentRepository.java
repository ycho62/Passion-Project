...
import com.golfkey.myapp.domain.Comment;
...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query(
        value = "select distinct comment from Comment comment left join fetch comment.clubs",
    ...
    @Query("select distinct comment from Comment comment left join fetch comment.clubs")
    List<Comment> findAllWithToOneRelationships();

    @Query("select comment from Comment comment left join fetch comment.clubs where comment.id =:id")
